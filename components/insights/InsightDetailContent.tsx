...
import { useInsightBySlug, usePublicInsights } from '@/lib/hooks/queries/useInsights';
import { PublicInsightDetail } from '@/lib/api/public-insights';
...
interface InsightDetailContentProps {
  slug: string;
  initialInsight: PublicInsightDetail | null;
  initialRecentInsights?: Array<{
    id: number;
    slug: string;
    title: string;
    summary: string;
...
export function InsightDetailContent({ slug, initialInsight, initialRecentInsights = [] }: InsightDetailContentProps) {
  // Use React Query with initial data from server
  const { data: insight, isLoading } = useInsightBySlug(slug, {
    initialData: initialInsight || undefined,
  });
  
  // Fetch recent insights
  const { data: recentData } = usePublicInsights({ limit: 5 });
  const recentInsights = recentData?.insights?.filter(i => i.slug !== slug).slice(0, 4) || initialRecentInsights;
        ...
        <article className="lg:col-span-5 order-1">
          ...
          <header className="mb-8">
            ...
            {insight.coverImageUrl && (
              <div className="relative w-full aspect-[16/9] md:aspect-[21/9] lg:aspect-[16/6] overflow-hidden rounded-lg mb-8">
                <InsightImage
